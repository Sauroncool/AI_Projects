# -*- coding: utf-8 -*-
"""
Created on Mon Mar  6 00:01:18 2023
                    ...
                    if block[i + 1][0] == '(':
                        for j in range(len(block[i + 1])):
                            if block[i+1][j]=='(':
                                ...
                                brac-=1
                            if (block[i + 1][j:j + 3] == 'and') and brac==0:
#                                print(block[i-1])
#                                print(block[i+1][1:j+1])
#                                print(block[i+1][j+4:])
                                temp[0] = '((' + block[i - 1] + ' or ' + block[i+1][1:j-1] + ')' + ' and ' + '(' + \
                                          block[i - 1] + ' or ' + block[i + 1][j + 4:] + ')'
#                                print(f'temp={temp}')
                                btemp = btemp[:i - 1] + temp + btemp[i + 2:]
                                print(btemp)
                                c=1
#                                print('break1')
                                break
...
#                    print('break3')
        ...
        if block == btemp:
            return btemp
        btemp = Proplogic.distribOR(btemp)
        #print(f'Intermediary Steps = {btemp}')
        return btemp
...
#s = 'p1 => (b or (c and d))'
#s = 'p1 or (b or (c and d)))'
s = 'a or (q1 and (c or d))) or e'
...
def do_all_in_loop(block0):
    ...
    block6=Proplogic.distribOR(block5)
    block6.reverse()
#    print(block6)
    block6=Proplogic.distribOR(block6)
    block6.reverse()

    if block6 == copy_of_block0:
        d=0   
        for i in range(len(block6)):
            ...
            for j in range(len(block6[i])):
                ...
                if c>1:
#                    print('ok finally')
                    blocktemp=block6[i][1:len(block6[i])-1]
                    li=Proplogic.findbracs(blocktemp)
                    some=do_all_in_loop(li)
                    block6 = block6[:i]+block6[i+1:]
                    some.reverse()
                    for m in range(len(some)):
                        block6.insert(i,some[m])
                    d=1
                    break
            if d==1:
                break
    else:
        do_all_in_loop(block6)
    return(block6)
...
result1 = do_all_in_loop(block0)
print('Final output:')
print(result1)
#for l in range(len(result1)):
#    if result1[l][0] == '(':
#        print('hola')
#        some = Proplogic.findbracs(result1[l])
#        print(do_all_in_loop(some))
